...
private control: Control;
    ...
    ngOnInit() {
        this.alarms = this.alarmData.getAlarmsBinding().pipe(map((alarms: any) => {
            return alarms.filter((a: any) => a.state <=1 && a.controlSerialNumber == this.control.serialNumber)
        }))
    ...
    public navigateToDetails(alarm: ControlAlarm) {
        this.nav.navigateForward('alarm-actions-page', {queryParams: { alarm: alarm }});
