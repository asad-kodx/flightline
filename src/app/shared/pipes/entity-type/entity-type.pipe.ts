...
export class EntityTypePipe implements PipeTransform {
    transform(_value: any, args: any[] | null | undefined): string {
        ...
        if (type === 'sensor') {
            const sensorType: SensorType = args[1];
            return this.getSensorTypeName(sensorType);
        }
        
        if (type === 'device') {
            const deviceType: DeviceType = args[1];
            return this.getDeviceTypeName(deviceType);
        }
        
        return 'Unknown';
    }
    
    private getSensorTypeName(sensorType: SensorType): string {
        switch (sensorType) {
